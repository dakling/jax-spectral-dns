...
class Case:
    ...
    def get_property_from_settings(
        ...
        with open(fname, "r") as file:
            ...
            return args.get(property, default)
...
def post_process(
    ...
    with h5py.File(file, "r") as f:
        ...
        for j in range(n_steps):
            ...
            energy_arr = np.vstack([np.array(ts), energy_t_arr])
        ...
        for i in range(n_steps):
            ...
            Nx = domain.get_shape()[0]
            if i == 0 or i == (n_steps - 1) // 2 or i == n_steps - 1:
                amplitudes_2d_kx = []
                kxs = np.array(list(range((Nx - 1) // 2 + 1)))
                en = vel.energy()
                for kx in kxs:
                    vel_2d_kx = vel_hat[0].field_2d(0, kx).no_hat()
                    amplitudes_2d_kx.append(vel_2d_kx.energy() / en)
                fig_spectral = figure.Figure()
                ax_spectral = fig_spectral.subplots(1, 1)
                ax_spectral.set_xlabel("$k_x$")
                ax_spectral.set_ylabel("$E_{k_{x}} / E$")
                ax_spectral.set_yscale("log")
                ax_spectral.plot(
                    2 * kxs, amplitudes_2d_kx, "ko"
                )  # TODO domain size hardcoded
                # ax_spectral.plot(
                #     2 * kxs, amplitudes_2d_kx, "k--", alpha=0.3
                # )  # TODO domain size hardcoded
                ax_spectral.set_ylim(top=1.0)
                fig_spectral.savefig(
                    Field.plotting_dir + "/plot_spectrum_kx_" + str(i) + ".png",
                    bbox_inches="tight",
                )
            ...
            # ax.plot(ts, energy_arr / energy_arr[0], "k-")
            # ax.plot(ts, energy_arr / energy_arr[0], "k.")
            # ax.plot(
            #     ts[: i + 1],
            #     energy_arr[: i + 1] / energy_arr[0],
            #     "ko",
            #     label="$G$",
            # )
            ...
            if True:
                ...
                ax_kx.set_ylim([0, 0.2])
                ax_kx_2.set_ylim([0, 3])
