...
def update_nonlinear_terms_high_performance(domain, vel_hat_new):
    ...
    hel_new = domain.cross_product(vel_new, vort_new) + 1/2 * jnp.array(vel_new_sq_nabla)
    ...
    visc_v = 1/1e5 * (domain.diff(lap_v, 0, 2) + domain.diff(lap_v, 1, 2) + domain.diff(lap_v, 2, 2))
        ...
        def perform_single_rk_step_for_single_wavenumber(
            ...
            def fn(K):
                ...
                def rk_00():
                    ...
                    N_00_new = (
                        jnp.block(
                            [-conv_ns_hat[0][kx__, :, kz__], -conv_ns_hat[2][kx__, :, kz__]]
                        )
                        - dPdx
                        * (dx * dz) ** (1 / 2)
                        * domain.aliasing
