...
def run_flow_sim_spectral(v0, grid, ii=-1):
    ...
    def energy_field(da, time_index):
        return (0.5*(da.data[time_index][0]**2 + da.data[time_index][1]**2))


    def energy_at_time(time_index):
        x, y = grid.axes()
        data = jnp.array(jnp.fft.irfftn(trajectory, axes=(1,2)))[time_index]
        print(data.shape)
        u, v = spectral_utils.vorticity_to_velocity(grid)(trajectory)
        energy_field = 0.5 * (u**2 + v**2)
        energy = jnp.trapz(jnp.trapz(energy_field, x, axis=0), y, axis=0)
        # energy = sum(sum(data)) # TODO this is probably incorrect!
        print(data)
        return energy
    ...
    # if True:
        ...
        plt.savefig("plot_u_" + str(ii) + ".pdf")
